    ...
    <div class="list-border-box">
      ...
      <template>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="16">
            <p>(1) 推广特色商户，获得特色商户平台销售额的0.3％；</p>
            <p>(2) 推广同城企业，获得同城企业平台销售额的0.3％；</p>
          </el-col>
        </el-row>
    ...
    <el-dialog :visible.sync="dialogVisible" title="平台服务费率变更" width="450px" center>
        <el-form ref="form" :model="form" label-width="120px">
          <el-form-item label="当前费率（%）">
            <span v-if="!!source.store && !!source.store.rateConfig">{{source.store.rateConfig.storeRatePlatform}}</span>
          </el-form-item>
    ...
    methods: {
      async handleOperate() {
        let res = await this.Api.updateRate({
          ...
          storeRatePlatform: this.source.store.rateConfig.storeRatePlatform * 1
        });
        if (res.status === 1) {
