<template>
   <div class="app-container">
      <div class="filter-container wrapper-box">
        ...
        <!-- <h2>添加生产企业</h2> -->
        <el-container class="info-form-wrapper">
          ...
          <el-container class="info-form-box">
            <el-aside width="200px">
              <p class="step-title-tag"></p>
            </el-aside>
            <el-main>
              <div class="info-form" v-show="active === 0">
                <h4 class="step-title-tag"><i class="icon-tag"></i>填写基本信息</h4>
                <el-form :model="shopInfo" label-width="250px" :rules="rules">
                  ...
                  <el-form-item label="营业期限：" required style="margin-bottom: 0;">
                  ...
                  <el-form-item label="法人姓名：" :prop="shopInfo.bcRegisterType === 2 ? '' : 'bcUserName'">
                    <el-input v-model="shopInfo.bcUserName"></el-input>
                  </el-form-item>
                  <el-form-item label="法人身份证号：" :prop="shopInfo.bcRegisterType === 2 ? '' : 'bcUserIdcard'">
                    <el-input v-model="shopInfo.bcUserIdcard"></el-input>
                  </el-form-item>
                  <el-form-item label="法人身份证正面：" :prop="shopInfo.bcRegisterType === 2 ? '' : 'idCardJustUrl'">
                    <selector key="idCardJustUrl" :dialog="subDialogIdcard" @submit="handleSelectIdcard"></selector>
                  </el-form-item>
                  <el-form-item label="法人身份证反面：" :prop="shopInfo.bcRegisterType === 2 ? '' : 'idCardBackUrl'">
                    <selector key="idCardBackUrl" :dialog="subDialogIdcardBack" @submit="handleSelectIdcardBack"></selector>
                  </el-form-item>
              ...
              <div class="info-form" v-show="active === 1">
                <h4 class="step-title-tag"><i class="icon-tag"></i>填写账号信息</h4>
                <el-form :model="source" label-width="250px" :rules="rules2">
                  <el-form-item label="店铺名称：" prop="nickName">
                    <el-input v-model="source.nickName" placeholder="省钱熊平台上展示的店铺名称"></el-input>
                  </el-form-item>
                  <el-form-item label="登录账号：" prop="userName">
                    <el-input v-model="source.userName" placeholder="6-16位，不能包含中文，不允许数字开头"></el-input>
                  </el-form-item>
                  ...
                  <el-form-item label="详细地址：" prop="address">
                    <el-input v-model="source.address"></el-input>
                  </el-form-item>
                </el-form>
              </div>
              <template v-if="active === 2">
                <div class="info-form" v-for="(m, inx) in shopCommunicationBookList" :key="m.role">
                  <h4 class="step-title-tag"><i class="icon-tag"></i>填写{{m.role}}</h4>
                  <el-form label-width="250px" style="padding-bottom: 20px;">
                    ...
                    <el-form-item label="邮箱：" v-if="inx < 4">
                      <el-input v-model="m.email"></el-input>
                    </el-form-item>
                  </el-form>
                </div>
              </template>
              <div class="info-form" v-show="active === 3">
                <h4 class="step-title-tag"><i class="icon-tag"></i>填写合同</h4>
                <el-form :model="rateConfig" label-width="250px" :rules="rules4">
                  ...
                  <el-form-item label="结算方式：" prop="settleType">
                    <el-radio-group v-model="source.settleType">
                      <el-radio :label="1">确认发货后结算</el-radio>
              ...
              <el-form label-width="250px" class="info-form-btn">
                <el-form-item label=" ">
                  <el-button type="primary" @click="prev" v-if="active !== 0">上一步</el-button>
                  <el-button type="primary" @click="next" v-if="active !== stepLength">下一步</el-button>
                  <el-button type="primary" @click="handleAdd" v-else>完成</el-button>
                </el-form-item>
              </el-form>
            </el-main>
          </el-container>
...
<script>
  ...
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          ...
          idCardJustUrl: [
            { required: true, message: '法人身份证正面', trigger: 'blur' }
          ],
          idCardBackUrl: [
            { required: true, message: '法人身份证反面', trigger: 'blur' }
          ]
        },
        rules2: {
          userName: [
        ...
        subDialogAvatar: {
          ...
          tableType: 'radio',
          files: {},
          // fileCategoryCode: 'FILE19011205660913'
        }
      }
    },
    components: {
    ...
    methods: {
      ...
      handleSelectIdcard(row, type) {
        this.shopInfo.idCardJustUrl = row.url;
        this.subDialogIdcard.files = Object.assign({}, row);
      },
      handleSelectIdcardBack(row, type) {
        this.shopInfo.idCardBackUrl = row.url;
        this.subDialogIdcardBack.files = Object.assign({}, row);
      },
      ...
      handleSelectImgs(row, type) {
        this.shopInfo.bcPicUrls = Object.assign([], row);
        this.subDialogImgs.files = Object.assign([], row);
      },
      goBack() {
      ...
      async handleAdd() {
        ...
        if (shopInfo.bcStartTime && shopInfo.bcEndtime) {
          ...
          delete shopInfo.bcStartTime;
        }
        let communication = [];
        if (!this.shopCommunicationBookList) {
          this.shopCommunicationBookList = [];
        }
