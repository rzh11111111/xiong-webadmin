    ...
    <div class="filter-container">
      ...
      <dialog-sub :dialog="dialog" :getDataList="getDataList" @submit="handleSave" @reset="handleCancel">
        <template slot="dialog">
          <tr>
            <td><i class="red-star" data-message="请选择线上店铺" data-name="shopIds">*</i>线上店铺：</td>
            <td>
          ...
          <!-- <el-form-item label="企业" prop="shopIds">
            <check-module ref="checkmodule" :dialog="subDialog" :disabled="dialog.readBool" @submit="handleSelect"></check-module>
          </el-form-item>
          ...
          <el-form-item label="阅读时间" prop="readTime" v-if="dialog.readBool && dialog.ruleForm2.isRead">
            <el-date-picker
...
export default {
  data() {
    ...
    return {
      ...
      dialog: {
        ...
        rules2: {
          shopIds: this.$plug.valid.isNull(),
          message: this.$plug.valid.isNull()
        }
      },
      subDialog: Object.assign({}, subDialog),
      subDialogMsg: Object.assign({}, subDialogMsg)
    }
  },
  components: {
    DialogSub,
  ...
  methods: {
    async getDataList() {
      ...
      let resData = await this.Api.getShopMessage(para);
    ...
    // },
    // async getInfo(id) {
    //   let res = await this.Api.getShopMessageDetail({id});
    //   if (res.status == 1) {
    //     this.dialog.ruleForm2 = res.data;
    //   }
    // },
    handleSelect(row, type) {
      // this.dialog.ruleForm2.shop = {
      //   id: row.id
      // };
      let shopIds = row.map(item => {
        return item.id;
      });
      this.dialog.ruleForm2.shopIds = shopIds.join(',');
    },
    ...
    async handleSave(row, savePrompt) {
      row.isRead = 0;
      // if(row.isRead){
      //   row.readTime = format(new Date(row.readTime),"yyyy-MM-dd hh:mm:ss");
      // }
      // if (!row.id) {
        savePrompt(this.Api.saveBatchShopMessage, row, this.moduleInit);
      // } else {
    ...
    async saveIsRead(row) {
      let res = await this.Api.readShopMessage({
        'id': row.id
      });
      if (res.status == 1) {
        this.$message({
