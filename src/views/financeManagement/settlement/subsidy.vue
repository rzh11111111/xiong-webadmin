  ...
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <table-sub ref="tableSub" :getDataList="getDataList" :table="table" :pagination="pagination">
        ...
        <template slot="platformRateSubsidy" slot-scope="{ index,row }">
          <span v-if="!!row.platformRateSubsidy">{{fixedNum(row.platformRateSubsidy * 100)}}%</span>
        </template>
        <template slot="moneyPayable" slot-scope="{index, row}">
          <span>{{(row.moneyPayable / 100)|currency}}</span>
        </template>
        <template slot="moneySubsidy" slot-scope="{index, row}">
          <span>{{(row.moneySubsidy / 100)|currency}}</span>
        </template>
