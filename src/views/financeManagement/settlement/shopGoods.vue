    ...
    <div class="filter-container">
      ...
      <table-sub ref="tableSub" :getDataList="getDataList" :table="table" :pagination="pagination">
        ...
        <template slot="shopType" slot-scope="{ index,row }">
          <span>{{row.shopType === 1 ? '生产企业' : '同城企业'}}</span>
        </template>
        <template slot="payOrder" slot-scope="{ index,row }">
          <span v-if="!!row.payOrder">{{row.payOrder.payOrderNo}}</span>
        </template>
        <template slot="platformRate" slot-scope="{ index,row }">
          <span v-if="!!row.platformRate">{{fixedNum((1 - row.platformRate) * 100)}}%</span>
        </template>
  ...
  export default {
    ...
    methods: {
      async getDataList() { //获取列表
        ...
        let res = await this.Api.getShopSettlementPage(para);
        return res;
      }
    }
  }
