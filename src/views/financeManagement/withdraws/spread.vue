  ...
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <table-sub ref="tableSub"
        ...
        <template slot="tallageIncrementMoney" slot-scope="{index, row}">
          <span>{{(row.tallageIncrementMoney / 100)|currency}}</span>
        </template>
        <template slot="tallageAttachMoney" slot-scope="{index, row}">
          <span>{{(row.tallageAttachMoney / 100)|currency}}</span>
        </template>
  ...
  export default {
      ...
      return {
        ...
        table: {
          ...
          columns: [{
            ...
            value: 'tallageIncrementMoney',
            label: '增值税',
            width: '140',
            slotBool: true
          }, {
            value: 'tallageAttachMoney',
            label: '附加税',
            width: '140',
            slotBool: true
          }, {
    ...
    methods: {
      async getDataList() { //获取列表
        ...
        let res = await this.Api.getSpreadWithdrawsPage(para);
        return res;
      },
      ...
      handleAgree(row, type) {
        ...
        this.$plug.prompt.del(this.agree, {extractBrokerageOrderIds: row.id, isRelease: '' + type}, null, type == 1 ? '同意提现' : '拒绝提现');
      },
      submitAgree() {
        ...
        this.$plug.prompt.del(this.agree, {extractBrokerageOrderIds: this.remarksDialog.id, isRelease: '0', disposeRemarks: this.remarksDialog.disposeRemarks}, null, '拒绝提现');
      },
      async agree(params) {
        let res = await this.Api.agreeSpreadWithdraw(params);
        if (res.status === 1) {
      ...
      handleBatch(row, delPrompt) {
        if (this.selectStatus == 1) {
          delPrompt(this.Api.agreeSpreadWithdraw, {extractBrokerageOrderIds: row.join(','), isRelease: '1'});
        } else if(this.selectStatus == 2) {
          delPrompt(this.Api.paymentSpreadWithdraw, {extractBrokerageOrderIds: row.join(','), isRelease: '1'});
        }
      },
      handlePayment(row, type) {
        this.$plug.prompt.del(this.payment, {extractBrokerageOrderIds: row.id, isRelease: '' + type}, null, type == 1 ? '打款成功' : '打款失败');
      },
      async payment(params) {
        let res = await this.Api.paymentSpreadWithdraw(params);
        if (res.status === 1) {
          this.$plug.prompt.saveTip('操作成功');
          this.$refs.tableSub.loadData();
        }
