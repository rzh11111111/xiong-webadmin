    ...
    <div class="filter-container">
      ...
      <table-sub ref="tableSub"
        ...
        <template slot="status" slot-scope="{ index,row }">
          <el-tag :type="statusColor[row.status - 1]" v-if="!!row.status">{{status[row.status * 1 - 1]}}</el-tag>
        </template>
       ...
       <!--  <template slot="extractType" slot-scope="{index, row}">
        ...
        <template slot="extractNickname" slot-scope="{index, row}">
          <span v-if="row.extractType == 2 && !!row.store">{{row.store.nickName}}</span>
        </template>
        ...
        <template slot="moneyGoodsReceivable" slot-scope="{index, row}">
          <span v-if="!!row.storeBankrollAccount">{{(row.storeBankrollAccount.moneyGoodsReceivable / 100)|currency}}</span>
        </template>
