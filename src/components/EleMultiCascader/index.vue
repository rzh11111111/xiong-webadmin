  ...
  <div class="cascader-wrapper">
    ...
    >
      ...
      <div class="cascader-menu-wrapper" v-clickoutside="hidePopover">
        <ul v-if="options.length > 0" class="el-cascader-menu cascader-menu"
  ...
  export default {
    ...
    methods: {
      initOpts() {
        ...
        if (this.isNational) {
          this.casTree = [this.clonedOpts[0].children];
        } else {
      ...
      checkedChange(item, checked) {
        ...
        if (item.code === '0') {
          this.checkAll = checked ? item : null;
      ...
      // 同步数据到上层
      syncData() {
        this.$emit("input", this.selectedValues, this.checkAll);
        this.$emit("change", this.selectedValues, this.selectedItems, this.checkAll);
      },
      // 展开下一级
      ...
      disabledChildren(item) {
        if (item.children instanceof Array) {
          for (var j = 0; j < item.children.length; j++) {
            item.children[j].disabled = true;
          }
        }
      },
      isDisable(item){
        if(item) {
          if (item.disabled) {
            this.disabledChildren(item);
            return true;
          }
          for(let i = 0; i < this.arrIsDisable.length; i++) {
            if(this.arrIsDisable[i].code === item.code) {
              this.disabledChildren(item);
              return true;
            }
          }
